...
int rwpng_read_image(FILE *infile, mainprog_info *mainprog_ptr)
{
...
#else
        fprintf(stderr, "pngnq readpng:  image is neither RGBA nor GA\n");
        png_destroy_read_struct(&png_ptr, &info_ptr, NULL);
    ...
    if(png_get_valid(png_ptr, info_ptr, PNG_INFO_bKGD))
      {
	mainprog_ptr->have_bg = TRUE;
	png_get_bKGD(png_ptr, info_ptr, &background);
	mainprog_ptr->bg_red = background->red;
	mainprog_ptr->bg_green = background->green;
	mainprog_ptr->bg_blue = background->blue;
      }
    ...
    num_comments = png_get_text(png_ptr, info_ptr,
				&comments, NULL);
    if(num_comments){
      mainprog_ptr->have_text = 1;
      for(c = 0; c < num_comments; c++){
	if(!strncmp("author",comments[c].key,6)){
	  mainprog_ptr->author = comments[c].text;
	}
      }
      };
